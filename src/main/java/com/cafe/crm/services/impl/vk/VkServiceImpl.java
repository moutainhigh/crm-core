...
public class VkServiceImpl implements VkService {
	...
	@Autowired
	public VkServiceImpl(TemplateService templateService, RestTemplate restTemplate, VkProperties vkProperties, EmailService emailService, UserService userService, PositionService positionService) {
	...
	private void checkForInvalidToken(ResponseEntity<String> response) {
		...
		if (jsonObject.has("error")) {
			...
			if (code == ERROR_CODE_INVALID_TOKEN) {
				List<User> users = userService.findByPositionName(EMAIL_RECIPIENT_ROLE_IN_CASE_ERROR);
				if (!users.isEmpty()) {
